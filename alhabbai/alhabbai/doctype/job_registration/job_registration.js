...
frappe.ui.form.on('Job Registration', {
    refresh: function(frm) {
        ...
        if (frm.doc.__islocal) {
            console.log("🆕 New document detected. Running initial workflow status check.");
        ...
        if (frm.doc.docstatus === 0 && frm.doc.custom_workflow_status === "Pending" && frappe.user_roles.includes("Verifier")) {
            ...
            frm.add_custom_button(__("Verify"), function() {
                frappe.confirm(
                    __("This will verify the document and create a draft purchase invoice. Continue?"),
                    function() {
    ...
    advance_payment: function(frm) {
        ...
        let pending = (frm.doc.total_amount || 0) - (frm.doc.advance_payment || 0);
...
function checkChildTableEntries(frm) {
    const hasDocumentsEntries = frm.doc.table_tujy && Array.isArray(frm.doc.table_tujy) && frm.doc.table_tujy.length > 0;

    console.log("🔍 DEBUG: Documents entries present:", hasDocumentsEntries);

    if (frm.doc.docstatus === 0) {
        if (hasDocumentsEntries) {
            if (frm.doc.custom_workflow_status !== "Pending") {
                console.log("🔁 Setting status to Pending (Documents exist)");
                frm.set_value("custom_workflow_status", "Pending");
                frm.refresh_field("custom_workflow_status");
            }
        } else {
            if (frm.doc.custom_workflow_status !== "Draft") {
                console.log("🔁 Setting status to Draft (No documents)");
...
function create_government_purchase_invoice(frm, amount = null) {
    frappe.call({
        ...
        callback: function(r) {
            if (r.message) {
                frappe.msgprint(`Purchase Invoice ${r.message} created for Government Fees.`);
            }
        }
    });
}
